...
class TwilioService:
    ...
    def _twilio_daily_limit_ok(self):
        ...
        count = NotificationLog.objects.filter(sent_at__date=today).count()
        if count >= limit:
            return (
                False,
                f"Límite diario de mensajes alcanzado ({limit}). "
                "Ajusta el valor en Configuración global del sistema.",
            )
        return True, None
